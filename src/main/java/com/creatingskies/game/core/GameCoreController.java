...
public class GameCoreController extends ViewController {
	...
	public void initialize() {
		...
		map = mapDao.findMapWithDetails(7);
	...
	private void initCountdownTimer(){
		...
		countDownTimer.getKeyFrames().add(new KeyFrame(Duration.seconds(1),
	...
	private void checkWarning(Shape block){
		...
		obstacleSlowLabel.setText(obstacleSlowFactor.toString());
	}
	
	private void checkGameStatus(Shape block){
		Shape intersect = Shape.intersect(block, end);
		if (intersect.getBoundsInLocal().getWidth() != -1) {
			timeline.stop();
			float result = millisGameDuration / 1000.0f;
	...
	public void createObstacleEdge(Tile tile){
		...
		obstacleEdge.setFill(Color.BLACK);
	...
	public void createEndRectangle(Tile tile){
		end = createDefaultRectangle(tile);
	...
	private void loadPlayer(){
		playerCircle = new Circle((Constant.TILE_WIDTH/2), Color.RED);
